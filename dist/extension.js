			...
			tagName(source, onError) {
				...
				if (source[1] === "<") {
					...
					if (verbatim === "!" || verbatim === "!!") {
						onError(
							`Verbatim tags aren't resolved, so ${source} is invalid.`
						);
						return null;
					}
			...
			tagString(tag) {
				for (const [handle, prefix] of Object.entries(this.tags)) {
					if (tag.startsWith(prefix))
						return (
							handle + escapeTagName(tag.substring(prefix.length))
						);
				}
	...
	"node_modules/.pnpm/yaml@2.6.1/node_modules/yaml/dist/nodes/Collection.js"(
		...
		var Collection = class extends Node.NodeBase {
			...
			addIn(path, value) {
				if (isEmptyPath(path)) this.add(value);
					...
					if (identity.isCollection(node)) node.addIn(rest, value);
						...
						this.set(
							key,
							collectionFromPath(this.schema, rest, value)
						);
					else
						throw new Error(
			...
			getIn(path, keepScalar) {
				...
				const node = this.get(key, true);
				if (rest.length === 0)
					return !keepScalar && identity.isScalar(node)
						? node.value
						: node;
				else
...
var require_stringifyString = __commonJS({
	"node_modules/.pnpm/yaml@2.6.1/node_modules/yaml/dist/stringify/stringifyString.js"(
		...
		function doubleQuotedString(value, ctx) {
			...
			const indent =
				ctx.indent || (containsDocumentMarker(value) ? "  " : "");
			let str = "";
			let start = 0;
		...
		function blockString(
			...
			const literal =
				blockQuote === "literal"
					? true
					: blockQuote === "folded" ||
						  type === Scalar.Scalar.BLOCK_FOLDED
						? false
	...
	"node_modules/.pnpm/yaml@2.6.1/node_modules/yaml/dist/stringify/stringify.js"(
		...
		function getTagObject(tags, item) {
			if (item.tag) {
				...
				if (match.length > 0)
					return (
						match.find(t => t.format === item.format) ?? match[0]
					);
			}
			let tagObj = void 0;
			let obj;
			if (identity.isScalar(item)) {
				...
				obj = item;
				tagObj = tags.find(
					t => t.nodeClass && obj instanceof t.nodeClass
				);
			}
			if (!tagObj) {
		...
		function stringifyProps(node, tagObj, { anchors: anchors$1, doc }) {
			...
			const props = [];
			const anchor =
				(identity.isScalar(node) || identity.isCollection(node)) &&
				node.anchor;
...
${ctx.indent}`;
				...
				const flow =
					ctx.inFlow ?? value.flow ?? value.items.length === 0;
				if (hasNewline || !flow) {
...
${indent}${line}`
			...
			if (comment) {
				str +=
					"\n" +
					stringifyComment.indentComment(
		...
		function stringifyFlowCollection(
			...
			for (let i = 0; i < items.length; ++i) {
				...
				if (
					!reqNewline &&
					(lines.length > linesAtValue || str.includes("\n"))
				)
					reqNewline = true;
				lines.push(str);
			...
			if (identity.isSeq(seq)) {
				for (let i = 0; i < seq.items.length; ++i) {
					...
					if (identity.isPair(item)) continue;
						...
						const pair =
							item.items[0] ||
							new Pair.Pair(new Scalar.Scalar(null));
...
${cn.comment}`
					...
					seq.items[i] = identity.isPair(item)
						? item
						: new Pair.Pair(item);
				}
		...
		var timestamp = {
			...
			resolve(str) {
				...
				const [, year, month, day, hour, minute, second] =
					match.map(Number);
				const millisec = match[7]
					? Number((match[7] + "00").substr(1, 3))
					: 0;
				let date = Date.UTC(
					year,
					month - 1,
			...
			return tags.reduce((tags2, tag) => {
				...
				if (!tagObj) {
					...
					const keys = Object.keys(tagsByName)
						...
						.join(", ");
					throw new Error(
						`Unknown custom tag ${tagName}; use one of ${keys}`
					);
				}
			...
			if (doc.directives?.docEnd) {
				...
				if (dc) {
					if (
						...
						lines.push("");
					lines.push(
						stringifyComment.indentComment(commentString(dc), "")
					);
				}
			}
			return lines.join("\n") + "\n";
			...
			switch (key.type) {
				...
				case "flow-collection":
					for (const it of key.items) {
						...
						if (
							containsNewline(it.key) ||
							containsNewline(it.value)
						)
							return true;
					}
					return false;
		...
		function resolveBlockMap(
			...
			for (const collItem of bm.items) {
				...
				if (ctx.schema.compat)
					utilFlowIndentCheck.flowIndentCheck(
						bm.indent,
						key,
						onError
					);
			...
			if (commentEnd && commentEnd < offset)
				onError(
					commentEnd,
					"IMPOSSIBLE",
					"Map comment with trailing content"
				);
			...
			for (let i = 0; i < fc.items.length; ++i) {
				...
				if (!isMap && !sep && !props.found) {
					...
					if (isMap) {
						const map = coll;
						if (
							utilMapIncludes.mapIncludes(ctx, map.items, keyNode)
						)
							onError(
								keyStart,
								"DUPLICATE_KEY",
		...
		function composeCollection(CN, ctx, token, props, onError) {
			...
			if (token.type === "block-seq") {
				...
				if (lastProp && (!nl || nl.offset < lastProp.offset)) {
					const message =
						"Missing newline after block sequence props";
			...
			if (!tag) {
				...
				if (kt && kt.collection === expType) {
					ctx.schema.tags.push(
						Object.assign({}, kt, { default: false })
					);
					tag = kt;
				} else {
					if (kt?.collection) {
	...
	"node_modules/.pnpm/yaml@2.6.1/node_modules/yaml/dist/compose/compose-node.js"(
		...
		function composeNode(ctx, token, props, onError) {
			...
			switch (token.type) {
				...
				case "double-quoted-scalar":
				case "block-scalar":
					node = composeScalar.composeScalar(
						ctx,
						token,
						tag,
						onError
		...
		function composeAlias({ options }, { offset, source, end }, onError) {
			...
			const valueEnd = offset + source.length;
			const re = resolveEnd.resolveEnd(
				end,
				valueEnd,
				options.strict,
				onError
		...
		var Composer = class {
			constructor(options = {}) {
				...
				this.onError = (source, code, message, warning) => {
					...
					if (warning)
						this.warnings.push(
							new errors.YAMLWarning(pos, code, message)
						);
					else
						this.errors.push(
							new errors.YAMLParseError(pos, code, message)
						);
				};
				this.directives = new directives.Directives({
			...
			*parsePlainScalar() {
				...
				while ((ch = this.buffer[++i])) {
					if (ch === ":") {
						...
						if (
							isEmpty(next) ||
							(inFlow && flowIndicatorChars.has(next))
						)
							break;
						end = i;
					} else if (isEmpty(ch)) {
			...
			*pushIndicators() {
				switch (this.charAt(0)) {
					...
					case ":": {
						...
						if (
							isEmpty(ch1) ||
							(inFlow && flowIndicatorChars.has(ch1))
						) {
			...
			*blockMap(map) {
				...
				switch (this.type) {
					case "newline":
						...
						if (it.value) {
							const end =
								"end" in it.value ? it.value.end : void 0;
							const last = Array.isArray(end)
								? end[end.length - 1]
								: void 0;
				...
				if (this.indent >= map.indent) {
					const atMapIndent =
						!this.onKeyLine && this.indent === map.indent;
					const atNextItem =
						atMapIndent &&
...
var require_public_api = __commonJS({
	"node_modules/.pnpm/yaml@2.6.1/node_modules/yaml/dist/public-api.js"(
		...
		function parseAllDocuments(source, options = {}) {
			const { lineCounter: lineCounter2, prettyErrors } =
			...
			if (prettyErrors && lineCounter2)
				for (const doc of docs) {
					doc.errors.forEach(
						errors.prettifyError(source, lineCounter2)
					);
		...
		function stringify(value, replacer, options) {
			...
			return new Document.Document(value, _replacer, options).toString(
				options
			);
		}
...
var AzurePipelinesTaskValidator = class {
	...
	async validatePipelineContent(document) {
	...
	normalizeTaskDefinition(taskInfo) {
		if (
			!(
				taskInfo.azurePipelineTaskDefinition instanceof
	...
	async validatePipelineTasks(yamlContent, diagnostics, document) {
		const processNode = async (obj, path = []) => {
	...
	async validateTask(taskObj, diagnostics, document) {
		const fullTaskName = taskObj.task;
		const taskInputsInYaml = taskObj.inputs || {};
	...
	findTaskLineInDocument(document, taskName) {
	...
	addDocumentDiagnostic(diagnostics, lineIndex, message, severity) {
		const range = new vscode.Range(lineIndex, 0, lineIndex, 1);
		const diagnostic = new vscode.Diagnostic(range, message, severity);
		diagnostic.source = this.extensionSource;
		diagnostics.push(diagnostic);
	}
	addDiagnostic(diagnostics, lineIndex, message, severity) {
	...
	validateRequiredInput(
		...
		const inputDefinition =
			taskDefinition.getInputDefinition(requiredInputName);
...
// src/services/VSCodeTaskCacheService.ts
var VSCodeTaskCacheService = class {
	// 24 hours
	constructor(context) {
	...
	async getCachedTasks() {
		...
		const cachedTimestamp = this.context.globalState.get(
		...
		return new Promise((resolve, reject) => {
			https
				.get(url, response => {
					...
					response.on("end", () => {
						try {
							const azurePipelinesTaskDefinition = Object.assign(
								new AzurePipelinesTaskDefinition(
									JSON.parse(data)
								),
								JSON.parse(data)
							...
							const isinstance =
								azurePipelinesTaskDefinition instanceof
								AzurePipelinesTaskDefinition;
							resolve(azurePipelinesTaskDefinition);
						} catch (error) {
							reject(error);
						}
					});
...
// src/core/AzurePipelinesExtension.ts
var AzurePipelinesExtension = class _AzurePipelinesExtension {
	static {
		this.EXTENSION_SOURCE = "Azure Pipelines Task Validator";
	}
	constructor(context) {
		this.diagnosticCollection =
			vscode2.languages.createDiagnosticCollection(
				_AzurePipelinesExtension.EXTENSION_SOURCE
			);
		this.context = context;
	...
	async activate() {
		...
		vscode2.workspace.textDocuments.forEach(
		...
		this.context.subscriptions.push(
			...
			vscode2.workspace.onDidSaveTextDocument(
				this.validateYAMLDocument.bind(this)
			)
		);
	}
	async validateYAMLDocument(document) {
